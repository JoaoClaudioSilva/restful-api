...
router.delete("/:nome", isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (linhas > 0)
      res.status(200).send("Requisição bem sucedida. Usuário excluído");
...
router.put("/update/:nome", isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (linhas > 0)
      res
        .status(200)
        .send("Requisição bem sucedida. Usuário alterado por admin");
    else res.status(404).send("Usuário não foi encontrado");
  } catch (err) {
    return errorHandler(res, err);
  }
...
router.put("/create/:nome", isAdmin, async (req, res) => {
  ...
  try {
    const linhas = await alteraUsuario(
      { nome: nome, isAdmin: false },
      { isAdmin: true }
    );
    ...
    if (linhas > 0)
      res
        .status(200)
        .send("Requisição bem sucedida. Usuário agora é administrador");
