...
router.get("/", async (req, res) => {
  try {
    const tabelas = await leTodasTabelas();
...
router.put("/:modelo/:id", helper_acesso.isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (linhas > 0)
      res.status(200).send("Requisição bem sucedida. Tupla atualizada");
...
router.delete("/:modelo/:id", helper_acesso.isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (linhas > 0)
      res.status(200).send("Requisição bem sucedida. Tupla excluída");
    else res.status(404).send("A tupla não foi encontrada");
  } catch (err) {
    return errorHandler(res, err);
  }
