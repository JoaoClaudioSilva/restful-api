...
const loja = Joi.object({
  ...
  nme_loja: Joi.string().required(),

  end_loja: Joi.string().required(),
});

const produto = Joi.object({
  id: Joi.number().min(0).required(),

  dsc_produto: Joi.string().required(),

  mrc_produto: Joi.string().required(),
});

const estoque = Joi.object({
  fk_produto: Joi.number().min(0).required(),

  fk_loja: Joi.number().min(0).required(),

  qnt_estoque: Joi.number().required(),

  prc_estoque: Joi.number().required(),
});
